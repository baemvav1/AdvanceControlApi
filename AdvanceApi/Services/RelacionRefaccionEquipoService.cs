...
{
    /// <summary>
    /// Implementación del servicio de relaciones refacción-equipo que usa el procedimiento almacenado sp_relacionEquipoCliente_edit
    /// </summary>
    public class RelacionRefaccionEquipoService : IRelacionRefaccionEquipoService
    {
            ...
            try
            {
                ...
                while (await reader.ReadAsync())
                {
                    var relacion = new RelacionRefaccionEquipo
                    {
                        ...
                        Costo = reader.IsDBNull(reader.GetOrdinal("costo")) ? null : Convert.ToDouble(reader.GetValue(reader.GetOrdinal("costo"))),
        ...
        public async Task<object> DeleteRelacionAsync(int idRelacionRefaccion)
        {
            try
            {
                ...
                _logger.LogDebug("Relación eliminada con id {IdRelacionRefaccion}", idRelacionRefaccion);
        ...
        public async Task<object> UpdateNotaAsync(RelacionRefaccionEquipoQueryDto query)
        {
            ...
            try
            {
                ...
                _logger.LogDebug("Nota actualizada para relación {IdRelacionRefaccion}", query.IdRelacionRefaccion);
